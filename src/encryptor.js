...
var getParamLength = function(methodName) {
  return cryptoParamLength[methodName];
};
module.exports.getParamLength=getParamLength;
...
var generateKey = function(methodName, secret) {
  ...
  while (length < keyLength) {
    hash = getMD5Hash(
      i === 0 ? secretBuf : Buffer.concat([tokens[i - 1], secretBuf])
    );
    tokens.push(hash);
    i++;
    length += hash.length;
